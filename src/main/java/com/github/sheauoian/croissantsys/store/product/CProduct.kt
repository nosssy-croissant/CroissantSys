...
class CProduct(val price: List<PriceType>, val iconItemStack: ItemStackSerializable, val product: CProductType) {
    companion object {
        fun createSimple(item: ItemStack, money: Int): CProduct {
            val item = item.clone()
            val price = listOf(MoneyPriceType(money))
            val product = ItemCProductType(item)
            return CProduct(price, item, product)
        }

        fun createSimple(equipmentData: EquipmentData, money: Int): CProduct {
            val price = listOf(MoneyPriceType(money))
            val product = WearingCProductType(equipmentData.id)
            return CProduct(price, equipmentData.item, product)
        }
    }
